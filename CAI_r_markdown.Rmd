...
#### Cancer Diagnoses
I think in general we want to describe patients' diagnoses because we want to know how those diagnoses affect some outcome variable of interest, be it life span or time in remission. That means we don't just want to describe a patient's dignoses in all dimensions, but along those dimensions that are most predictive of outcomes of interest.


To this end, I would want to take an increasingly in-depth look at two main things: 1.) the most common diagnoses across the entire population and 2.) all of the diagnoses that the individual patient has. Together, these two things will start to inform us on how each patient is "typical" and how they are unique in their diagnoses. Further, taking a careful look at the "most informative" diagnosis variables can form a strong foundation for feature selection in later modeling endeavors.

I think one good place to start is likely the condition table and then bring in more and more data elements, such as biomarkers, to help create a detailed characterization of each patient that eventually extends to include medications, procedures, Labs, chemo therapies, tumor size, lymph node counts, and metastasis information. 

With this rough roadmap in mind, I would first take a look at an individual patient's diagnoses to start building a mental model of what types of diagnoses someone might have. I'll start with patient 29 (who has a death date as 2020-06-28):
```{r}
filter(condition, patient_id == 29) %>% # limit to patient 21
...
```
#### Notes: 
What I see here is a patient who had a first (at least in this data) diagnosis of a malignant neoplasm in the right breast. The cancer then spread to the bone, the left breast, the left lung, and ultimately the brain in late 2019. The patient halfway through the following year. But even in this relatively short diagnosis history there are a lot of other things I would want to know. 
